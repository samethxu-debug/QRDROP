...
import { X, ShieldCheck, AlertCircle, Sparkles, Shield, Lock, Mail, ArrowRight } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose, onAuthSuccess, t }) {
  const [authTab, setAuthTab] = useState('google'); // 'google' | 'admin' | 'manual'
  ...
  // Admin credentials
  const [adminEmail, setAdminEmail] = useState('samethxu@gmail.com');
  ...
  const handleAdminSubmit = async (e) => {
    ...
    try {
      const res = await safeFetchJson('/api/auth/admin-login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  useEffect(() => {
    if (!isOpen || authTab !== 'google') return;
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <h2 className="text-xl sm:text-2xl font-black text-white">
            {authTab === 'admin' ? 'Admin Access' : (t.googleSignInTitle || 'Sign in with Google')}
          </h2>
          <p className="text-xs text-slate-400 max-w-xs mx-auto">
            {authTab === 'admin' 
              ? 'Administrator account login for platform management.' 
              : (t.googleSignInSubtitle || 'Sign in with your Google account to upload and send files securely.')}
          </p>
        </div>

        {/* Tab Switcher: Google vs Admin */}
        ...
        {/* TAB 1: Google Login (Shows EXACTLY ONE clean Google button) */}
        {authTab === 'google' && (
          <div className="space-y-4">
            
            {/* If GSI SDK is ready, show GSI button; else show single direct Google button */}
            <div className="flex justify-center">
        ...
        {/* TAB 2: Manual Specific Google Account */}
        {authTab === 'manual' && (
          <form 
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            ...
            <div className="pt-2 flex items-center justify-between gap-2">
              ...
              >
                {loading ? (t.submitting || 'Signing in...') : (t.signInButton || 'Sign In')}
              </button>
            </div>
          </form>
        )}

        {/* TAB 3: Admin Login (Google Sign-In + Password) */}
        {authTab === 'admin' && (
          <div className="space-y-4">
            {/* 1-Click Admin Google Sign In */}
            <button
              type="button"
              disabled={loading}
              onClick={() => handleDirectGoogleLogin('samethxu@gmail.com', 'Sameth Admin')}
              className="w-full py-3 px-4 rounded-2xl bg-white hover:bg-slate-100 text-slate-900 font-extrabold text-xs shadow-xl flex items-center justify-center gap-2.5 transition disabled:opacity-50 cursor-pointer"
            >
              ...
              <span>{loading ? 'Authenticating Admin...' : 'Sign in as Admin with Google'}</span>
            </button>

            <div className="flex items-center gap-3 my-1">
              <div className="h-px bg-slate-800 flex-1" />
              <span className="text-[10px] text-slate-500 font-semibold uppercase">Or Admin Password</span>
              <div className="h-px bg-slate-800 flex-1" />
            </div>
            ...
            <form onSubmit={handleAdminSubmit} className="space-y-3 text-left">
              <div>
                ...
                <input
                  ...
                  placeholder="samethxu@gmail.com"
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-purple-500 transition font-mono"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-purple-500 transition"
                  autoFocus
                />
              </div>
              ...
              <div className="pt-1">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-2.5 px-4 rounded-2xl bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-500 hover:from-purple-500 hover:to-indigo-400 text-white font-extrabold text-xs shadow-xl shadow-purple-500/20 flex items-center justify-center gap-2 transition disabled:opacity-50 cursor-pointer"
                >
